...
// 파라미터가 옵션일 경우
function my_func2(data? :number) :void{ // data? :number는 data :number | undefined와 같다
...
function text_func(param? :string) : void{
    if(!param){
...
function text_func2(param : number) :number{
    const temp_data :string = param.toString();
...
function text_func3(money :number, home :boolean, pretty :string){
    let temp_home :number;
    let temp_pretty :number;
...
function my_func4(x :number | string){
    if(typeof x === 'string'){
        return x + '1'
    } else {
        return x + 1
...
function my_func5( x:number | string){
    let array :number[] = [];
...
function my_func6( x:number | string){
    let array :number[] = [];
    array[0] = x as number;
}

// 문제 1. array 요소 중 문자 타입을 숫자로 변환해 출력
function change_func ( x:(number | string)[]){
    let array :number[] = [];
...
function change_func2( x :{subject : string | string[]} ){
